    ...
    , logStream(ostream)
{
    ...
    logStream << logLock.out << listenConn.ipv4_str() << " [Info]HTTP Server started"
              << logLock.endl;
}

HttpServer::~HttpServer()
{
    logStream << logLock.out << listenConn.ipv4_str() << " [Info]Stopping HTTP Server..."
              << logLock.endl;
    ...
    logStream << logLock.out << listenConn.ipv4_str() << " [Info]HTTP Server stopped"
              << logLock.endl;
}
    ...
    do {
        ...
        if (bytesReceived == 0)
            ...
            logStream << logLock.out << conn.ipv4_str()
                      << " [Erro]Recv failed with error: " << WSAGetLastError()
        ...
        try {
            ...
            if (request.headers.find("Content-Length") != request.headers.end()) {
                ...
                if (contentLength != request.body.length())
                    logStream << logLock.out << conn.ipv4_str() << " [Warn]Incomplete body(expect "
                              << contentLength << " bytes, actual " << request.body.length()
                              << " bytes)" << logLock.endl;
            }
            ...
            keepAlive =
                !(request.version == "HTTP/1.1" && request.lowerHeader("Connection") == "close"
                  || request.version == "HTTP/1.0"
                         && request.lowerHeader("Connection") != "keep-alive");
            ...
            logStream << logLock.out << conn.ipv4_str() << " [Info]Response(" << response.statusCode
                      << " " << response.statusInfo() << "): " << response.body.length() << " bytes"
                      << logLock.endl;
        }
        // 处理请求重定向异常
        catch (Redirect r) {
