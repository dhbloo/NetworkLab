...
int main()
{
    try {
        Router router;
        router.setRoute("/time", Request::GET, MakeFuncView([](auto &req, auto &res) {
                            res.body = Rfc1123DateTimeNow();
                        }));
        router.setRoute("/factorial/<n>", Request::GET, MakeFuncView([](auto &req, auto &res) {
                            try {
                                uint64_t n = std::stoi(req.urlParams["n"]);
                                for (uint64_t i = 2, m = n; i < m; i++)
                                    n *= i;
        ...
        router.setRoute("/<path:filepath>",
                        Request::GET,
                        std::make_shared<StaticFileView>(R"(C:\Users\dhb\Desktop\TestWeb)"));
        router.setErrorHandler(0, MakeFuncView([](auto &req, auto &res) {
