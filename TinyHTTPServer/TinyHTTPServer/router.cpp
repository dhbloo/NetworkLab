...
void Router::setRoute(std::string url, int supportedMethods, ViewPtr view)
{
    ...
    std::string urlRe(std::regex_replace(std::regex_replace(url, urlParamPathRe, "(\\S*)"),
                                         urlParamRe,
                                         "([^/]+)"));
...
{
    urlMap.erase(std::remove_if(urlMap.begin(), urlMap.end(), [&url](const Route &route) {
...
ViewPtr Router::resolve(Request &request, Response &response) const
{
    ...
    if (routeFinded) {
        ...
        throw Abort(405,
                    "Method " + getAllowedString(request.method) + std::string(" not allowed for ")
                        + request.url);
    }
...
{
    std::string allowed;
    if (methods & Request::GET)
        allowed.append("GET, ");
    if (methods & Request::HEAD)
        allowed.append("HEAD, ");
    if (methods & Request::POST)
        allowed.append("POST, ");
    if (methods & Request::PUT)
        allowed.append("PUT, ");
    if (methods & Request::DEL)
        allowed.append("DELETE, ");
    if (methods & Request::OPTIONS)
