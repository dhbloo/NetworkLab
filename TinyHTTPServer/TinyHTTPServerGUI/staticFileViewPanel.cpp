...
StaticFileViewPanel::StaticFileViewPanel(wxWindow* parent) : 
    ...
    lineSizer->Add(new wxStaticText(this, wxID_ANY, "访问目录时的默认文件名:"), 0, wxALL | wxALIGN_CENTER_VERTICAL, 5);
    lineSizer->Add(defFileText = new wxTextCtrl(this, wxID_ANY, "index.html"), 0, wxALL, 5);

    viewSizer->Add(lineSizer = new wxBoxSizer(wxHORIZONTAL));
    lineSizer->Add(new wxStaticText(this, wxID_ANY, "默认扩展名:"), 0, wxALL | wxALIGN_CENTER_VERTICAL, 5);
...
ViewPtr StaticFileViewPanel::getView() const {
    return std::make_shared<StaticFileView>(
        pathText->GetValue().ToStdString(),
